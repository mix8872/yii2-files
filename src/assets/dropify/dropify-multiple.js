...
DropifyMultiple.prototype.createElements = function()
{
    ...
    if (this.isDisabled === false && this.settings.showRemove === true) {
        ...
        this.elementTable = this.wrapper.siblings('table');
        this.elementTable.on('fileDeleted', this.deleteElement.bind(this));
    }
    ...
    this.settings.defaultFile = this.settings.defaultFile || '';

    if (this.settings.defaultFile.trim() !== '') {
        this.setDefaultFile();
    }
};

DropifyMultiple.prototype.deleteElement = function ()
{
    this.settings.defaultFile = '';
    this.clearElement();
}

DropifyMultiple.prototype.setDefaultFile = function()
{
    var f = this.cleanFilename(this.settings.defaultFile);
    this.filenameWrapper.children('.dropify-filename-inner').html('');
    this.setPreview(this.isImage(f), this.settings.defaultFile, '');
}
...
DropifyMultiple.prototype.readFile = function(input)
{
    ...
    for( var i=0; i < j; i++ ) {
        ...
        {
            this.onFileReady(false, false, file);
        }
    }
};
...
DropifyMultiple.prototype.setPreview = function(previewable, src, fileName)
{
    ...
    var clear = this.wrapper.children('.dropify-clear');

    clear.css('display', (fileName ? 'block' : 'none'));
    this.hideLoader();
    ...
    if (previewable === true) {
        ...
        $('<span class="dropify-extension" />').html(this.getFileType(fileName || src)).appendTo(render);
    }
    this.preview.fadeIn();
};
